...
public class AcceptedPropSupJDBC 
{
	public static String getData(String totalRows,String s_uname) throws SQLException
	{
		try{
			...
			ResultSet rs2 = st.executeQuery("select p.ProposalID,r.id,r.type,r.expected_delivery_date,p.Quote,p.Date,p.created_contract "
					+ "from proposals p,requirements r "
					+ "WHERE p.RequirementID=r.id AND p.SupplierID = "+sid+" AND p.status = 1;");
			...
			while(rs2.next())
			{
					totalRows += "{\"prop_id\": \"" + rs2.getString(1) + 
							...
							+ "\", \"date\": \"" + rs2.getString(6) + "\", \"created_contract\": \"" + rs2.getString(7) + "\"},";
			}
