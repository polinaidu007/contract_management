...
public class CreatedContractsSupJDBC 
{
	public static String getData(String totalRows,String s_uname) throws SQLException
	{
		try{
			...
			ResultSet rs2 = st.executeQuery("select c.ContractID,c.RequirementID,r.type,c.ContractType,c.ContractDuration "
					+ "from contracts c,requirements r "
					+ "WHERE c.RequirementID=r.id AND c.SupplierID = "+sid+" AND c.active_ = 0;");
			...
			while(rs2.next())
			{
					totalRows += "{\"contract_id\": \"" + rs2.getString(1) + 
							"\", \"requirement_id\": \"" + rs2.getString(2) + "\", \"requirement_type\": \"" + rs2.getString(3) +
							 "\", \"contract_type\": \"" + rs2.getString(4) +"\", \"contract_duration\": \"" + rs2.getString(5)
							+ "\"},";
			}
